...
export default function Encadrant_list_page() {
  ...
  const fetchEncadreur = useCallback(async () => {
    try {
      setLoading(true);
  ...
  useEffect(() => {
    dispacth(setBreadCrumbItemsAction([encadrant_routes_items.encadrant]));
    return () => {
  ...
  const ComfirmeDialogForCreateLotProduction = (e) => {
    ...
    confirmDialog({
      ...
      header: "Enregistrer un encadreur?",
      message: (
        <div className="d-flex flex-column align-items-center">
          <div className="text-center mt-5">
            Voulez-vous vraiment enregistrer un encadreur?
          </div>
        </div>
      ),
   ...
   const deleteItems = async (itemsIds) => {
          try {
            ...
            const res = await fetchApi("/rapport_stage/faculte_depar/delete", {
              method: "POST",
              body: form,
            });
            dispacth(
                    ...
                    <>
                      <div className="d-flex round-indicator">
                        ...
                        <div className="ml-2">
                          ...
                          <div className="text-muted">{item?.TITRE}</div>
                        </div>
                      </div>
                      <style>{css}</style>
                    </>
                  ...
                  const items = [

                    {
                      template: (deleteItem, options) => {
                        return (
                          <Link
                            to={`/encadrant/edit/${inViewMenuItem?.ID_ENCA}`}
