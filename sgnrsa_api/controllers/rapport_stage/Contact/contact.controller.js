...
const createcontact = async (req, res) => {
  try {
    ...
    const contact = await Contact.create({
       NOM, PRENOM, EMAIL,ID_STATUT_C:1,ID_ADMIN:1
...
const createMessage = async (req, res) => {
  try {
    ...
    const msg = await Messagerie.create({
      // NOM, PRENOM, EMAIL,MESSAGE,ID_STATUT_C:1,ID_ADMIN:1
      ID_ADMIN:1,ID_ETUD:1,MESSAGE,ID_CONTACT:1,STATUT:1,
    })
    res.status(RESPONSE_CODES.CREATED).json({
      statusCode: RESPONSE_CODES.CREATED,
      httpStatus: RESPONSE_STATUS.CREATED,
...
const findAllSMS = async (req, res) => {
  ...
  try {
    ...
    const sortColumns = {
      messagerie: {
        ...
        fields: {
          ...
          // EMAIL: 'EMAIL',
          MESSAGE: 'MESSAGE',
          DATE_INSERTION: 'messagerie.DATE_INSERTION'
        }
      }
    }
    ...
    if (!orderColumn || !sortModel) {
      orderColumn = sortColumns.messagerie.fields.DATE_INSERTION
      sortModel = {
        model: 'messagerie',
        as: sortColumns.messagerie.as
      }
    }
    ...
    const result = await Messagerie.findAndCountAll({
      limit: parseInt(rows),
      offset: parseInt(first),
      order: [
        [sortModel, orderColumn, orderDirection]
