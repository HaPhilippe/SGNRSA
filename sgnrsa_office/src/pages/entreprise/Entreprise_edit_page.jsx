...
export default function Entreprise_edit_page() {
  const dispacth = useDispatch();
  const [data, handleChange, setData, setValue] = useForm(initialForm);
  const [showCalendar, setShowCalendar] = useState(false);
  const [isSubmitting, setIsSubmitting] = useState(false);
  const navigate = useNavigate();
  const { ID_ENTREPR } = useParams();
  ...
  const handleSubmit = async (e) => {
    try {
      ...
      if (isValidate()) {
        ...
        const res = await fetchApi(`/rapport_stage/entreprise/update/${ID_ENTREPR}`, {
          method: "PUT",
          body: form,
        });
        dispacth(
          setToastAction({
            ...
            detail: "Entreprise a été modifié avec succès",
            life: 3000,
          })
        );
        navigate("/entreprise");
  ...
  useEffect(() => {
    (async () => {
      try {
        const res = await fetchApi(`/rapport_stage/entreprise/find/${ID_ENTREPR}`);
        const entreprise = res.result;
        // return console.log(dep,"entreprise");

        setEntreprise(entreprise);
        setData({
          NOM_ENTREPR: entreprise?.NOM_ENTREPR,
          ADRESSE_ENTREPR: entreprise?.ADRESSE_ENTREPR,
          SECTEUR:entreprise?.SECTEUR,
          NOM_TUT:entreprise?.NOM_TUT,
          PRENOM_TUT:entreprise?.PRENOM_TUT,
  ...
  useEffect(() => {
    dispacth(
      setBreadCrumbItemsAction([
        ...
        entreprise_routes_items.edit_entreprise
      ])
    );
    return () => {
