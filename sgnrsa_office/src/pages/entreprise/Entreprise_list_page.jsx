...
export default function Lot_production_list_page() {
  ...
  const fetchEntreprise = useCallback(async () => {
    try {
      setLoading(true);
      const baseurl = `/rapport_stage/entreprise/fetch?`;
      var url = baseurl;
  ...
  useEffect(() => {
    dispacth(setBreadCrumbItemsAction([entreprise_routes_items.entreprise]));
    return () => {
  ...
  const deleteItems = async (itemsIds) => {
    try {
      ...
      const res = await fetchApi("/rapport_stage/entreprise/delete", {
        method: "POST",
        body: form,
      });
      dispacth(
        setToastAction({
          severity: "success",
          summary: "Entreprise supprimé",
          detail: "L'entreprise a été supprimé avec succès",
          life: 3000,
        })
      );
      fetchEntreprise();
    ...
    <>
      ...
      <div className="px-4 py-3 main_content">
        <div className="d-flex align-items-center justify-content-between">
          <h1 className="mb-3">Entrprise</h1>
          <Button
            label="Nouveau"
            icon="pi pi-plus"
            size="small"
        ...
        <div className="shadow my-2 bg-white p-3 rounded d-flex align-items-center justify-content-between">
          ...
          <div className="selection-actions d-flex align-items-center">
            ...
            <a
              ...
              style={{}}
              onClick={(e) =>
                handleDeletePress(e, selectedItems.map((item) => item.ID_ENTREPR))
              }
            >
              <svg
              ...
              <Column
                ...
                body={(item) => {
                  ...
                  return (
                    ...
                    <>
                      <Tooltip target={`#Nocmnde-${item.NOM_ENTREPR}`} className="w-4">
                            ...
                            <div className="col-sm ml-1">
                              <div className="form-group col-sm">
                                <div className="row">
                                  ...
                                  <div className="col-sm">: {item?.NOM_ENTREPR}</div>
                                </div>
                              </div>
                              <div className="form-group col-sm">
                                <div className="row">
                      ...
                      <Link
                        id={`Nocmnde-${item?.ID_ENTREPR
                          }`}
                        className="text-decoration-none d-flex round-indicator"
                        style={{ color: '#399af2' }}
                        to={`/entreprise/add/${item?.ID_ENTREPR}`}
                        onClick={(e) => {
                          e.stopPropagation();
                        }}
                        data-pr-position="bottom"
                      >
                        <span>{item?.NOM_ENTREPR}</span>
                      </Link>
                    </>
                  );
                }}
                  ...
                  const items = [

                    {
                      template: (deleteItem, options) => {
                        return (
                          <Link
                            to={`/entreprise/edit/${inViewMenuItem?.ID_ENTREPR}`}
